...
addCommandHandler("updatecedit",function(plr)
    print("[LevelSystem] Łączenie z GitHub...")
    print("[LevelSystem] Przygotowywanie do pobierania...")
    fetchRemote("https://raw.githubusercontent.com/ShuffleCodes/LevelSystem/main/meta.xml",function(data,error,path)
        if error==0 then
            ...
            fileWrite(file,data)
            fileClose(file)
            print("[LevelSystem] Lista plików pobrana, szykuję do pobrania...")
            prepare()
        end
    end,"",false,"meta.xml")
end)
...
function prepare()
    ...
    print("[LevelSystem] Pliki przygotowane do pobrania...")
    xmlUnloadFile(xml)
    fileCount=0
    download()
end
...
function download()
    ...
    fetchRemote("https://raw.githubusercontent.com/ShuffleCodes/LevelSystem/main/"..files[fileCount],function(data,error,sciezka)
        if error==0 then
